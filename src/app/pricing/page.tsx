...
export default function PricingPage() {
  const [selectedBin, setSelectedBin] = useState(bins[1]); 
  ...
  return (
    <main className="max-w-6xl mx-auto p-8 text-white text-center bg-gradient-to-r from-gray-900 to-gray-800 rounded-xl shadow-2xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
        {bins.map((bin) => (
          <BinCard key={bin.id} bin={bin} selectedBin={selectedBin} setSelectedBin={setSelectedBin} setShowPayment={setShowPayment} />
        ))}
      </div>
      {!showPayment && <button onClick={handlePayment} className="bg-yellow-500 px-8 py-3 text-lg rounded-lg">Pay Now (${selectedBin.price})</button>}
      {showModal && <CustomerDetailsModal customerDetails={customerDetails} setCustomerDetails={setCustomerDetails} setShowModal={setShowModal} handleSubmitDetails={handleSubmitDetails} errorMessage={errorMessage} />}
